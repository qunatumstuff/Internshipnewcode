...
def run_yolo_detection(color_image, depth_frame, intrinsics):
    """
    Run OBB detection on all catalogue objects and segmentation on pipe/sponge.

    Uses camera.py's loaded YOLO models (camera.model and camera.segment)
    so we don't load duplicate model weights.

    Returns a list of detection dicts:
        object_name, x, y, z, angle_deg, confidence
        + grasp_label for pipe
    """
    detections = []
    ...
    for seg_result in seg_results:
        ...
        for mask, class_id, conf in zip(masks, class_ids, confs):
            ...
            if cls_name == "pipe":
                # Compute the two physical pipe ends, select best grasp
                best = _compute_pipe_grasp(
                    mask_bin, depth_frame, intrinsics, angle_rad
                )
                if best is None:
                    continue
                detections.append({
                ...
                # Sponge — use mask centre deprojected through depth
    ...
    for result in obb_results:
        ...
        for obb in result.obb:
            ...
            # Pipe and sponge handled above by segmentation
