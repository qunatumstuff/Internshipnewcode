...
CAM_TO_ROBOT_T = np.array([
    ...
    [-0.0349602509, -0.4652189343, -0.8865742149, 0.8232530084],
            ...
            for result in results:
                if result.obb is None:
                    ...
                    # Deproject pixel to 3D spatial coordinates [X, Y, Z] relative to camera
                        ...
                        #    points1=np.array([[x1, y1], [x2, y2], [x3,y3], [x4,y4]], dtype=np.float32)
                        #    points2 = np.array([[0, 0], [200, 0], [200,200], [0,200]], dtype=np.float32)
                        #    matrix=cv2.getPerspectiveTransform(points1, points2)
                        #    top_down=cv2.warpPerspective(color_image, matrix, (200,200))
                        #    cv2.circle(top_down, (center_x, center_y), 5, (0, 0, 255), -1)
