...
def vision_loop():
    ...
    try:
        while True:
            ...
            # Run YOLO inference
            results = model(color_image, verbose=False,agnostic_nms=True,iou=0.35,conf=0.50)
            pipe_detection = segment(color_image, verbose=False,agnostic_nms=True,iou=0.35,conf=0.50)
            current_boxes=[]
            for pipe in pipe_detection:
                if pipe.masks is not None:
                    ...
                    for mask, class_id, conf in zip(masks, class_ids, confidences):
                            ...
                            #angle=float(pipe.masks.angles[0][0])
            ...
            for result in results:
                ...
                for obb in result.obb:
                    ...
                    # Deproject pixel to 3D spatial coordinates [X, Y, Z] relative to camera
                       ...
                       cv2.putText(color_image, f"Robot Frame: X:{robot[0]*1000:.4f} Y:{robot[1]*1000:.4f} Z:{robot[2]*1000:.4f}m R:{roll/math.pi*180:.2f} P:{pitch/math.pi*180:.2f} Y:{yaw/math.pi*180:.2f} {cls_name}", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                       print(f"{cls_name} X:{robot[0]*1000:.4f} Y:{robot[1]*1000:.4f} Z:{robot[2]*1000:.4f}m R:{roll/math.pi*180:.2f} P:{pitch/math.pi*180:.2f} Y:{yaw/math.pi*180:.2f}")
                    # Check if this is the target System 2 told us to track
                       if current_target_class and cls_name.lower() == current_target_class.lower():
                        # Calculate center pixel of the bounding box
                        #center_x = int(obb.xywh[0][0])
                        #center_y = int(obb.xywh[0][1])
                        ...
                        # Display tracking info on screen
                          coord_text = f"Target {cls_name}: X:{spatial_coords[0]:.2f} Y:{spatial_coords[1]:.2f} Z:{spatial_coords[2]:.2f}m"
                          cv2.putText(color_image, coord_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                          print(f"[Tracking] {coord_text}")
                        
                        # TODO for student: Broadcast latest_3d_coords via UDP to Module A here
