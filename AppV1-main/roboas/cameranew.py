...
def mouse_click(event,x,y,flags,param):
    if event==cv2.EVENT_LBUTTONDOWN:
        if len(points)<4:
           points.append([x,y])
           #cv2.circle(current_rgb_frame,(x,y),5,(0,0,255),-1)
...
def vision_loop():
    ...
    global current_rgb_frame, current_target_class, latest_3d_coords, mouse_click, spatial_coords, points
    ...
    try:
        while True:
            ...
            for result in results:
                if result.obb is None:
                    ...
                    # Calculate center pixel of the bounding box
                        ...
                        #x1,y1,x2,y2,x3,y3,x4,y4,cls_name,confidence = current_boxes[0]
                        ...
                        #cv2.circle(cropped_color_image, (center_x, center_y), 2, (0, 0, 255), -1)
                    ...
                    # Deproject pixel to 3D spatial coordinates [X, Y, Z] relative to camera
                        ...
                        top=sorted(coordinates_list,key=lambda point:point[1])[:2] #top 2
                        bottom=sorted(coordinates_list,reverse=True, key=lambda point:point[1])[:2] #bottom 2
                        ...
                        c010 = (int(top_left[0] + 0.77 * (bottom_left[0] - top_left[0])),int(top_left[1] + 0.77 * (bottom_left[1] - top_left[1])))
                        c110 = (int(top_right[0] + 0.77 * (bottom_right[0] - top_right[0])),int(top_right[1] + 0.77 * (bottom_right[1] - top_right[1])))

                        center_top=np.mean([c010,c110,top_right,top_left],axis=0)
                        center_topx=int(center_top[0])
                        center_topy=int(center_top[1])

                        cv2.circle(color_image, (int(center_topx), int(center_topy)), 2, (0, 0, 255), -1)
                        ...
                        if len(points)==4:
                            ...
                            key=None
                            points1=np.array([obb.xyxyxyxy[0].cpu().numpy().astype(int)], dtype=np.float32)
                            points2 = np.array([[0, 0], [431, 0], [431,439], [0,439]], dtype=np.float32)
                            matrix=cv2.getPerspectiveTransform(points1, points2)
                            top_down=cv2.warpPerspective(color_image, matrix, (431,439))
                            centerpoints=cv2.perspectiveTransform(np.array([[[center_x,center_y]]],dtype="float32"),matrix)
                            center_x1=int(centerpoints[0][0][0])
                            center_y1=int(centerpoints[0][0][1])

                            cv2.circle(top_down, (center_x1, center_y1), 5, (0, 0, 255), -1)
                            color_image=top_down.copy()

                        #if key1 == ord('r'):
                        #       points=[]
                        #       revert_matrix=cv2.getPerspectiveTransform(points2,points1)
                        #       revert_color_image=cv2.warpPerspective(top_down,revert_matrix,(640,480))
                        #       color_image=revert_color_image.copy()
            ...
            cv2.setMouseCallback("Module B: System 1 Vision Reflex",mouse_click)
