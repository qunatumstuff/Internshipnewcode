...
- When the user asks you to pick up an object based on generic YOLO class alone (e.g. 'pick up the nut'), simply call the 'locate_object' tool.
- CRITICAL: If the user requests an object based on visual properties, branding, stickers, or implicit concepts (e.g., 'I am thirsty' -> milk, 'Pick the milk'), DO NOT call 'locate_object' immediately. First, call 'analyze_scene_semantics()' to map generic objects to their visual descriptions. Then, call 'locate_object' with 'target_name: "cube"' and 'user_context: "milk sticker"'.
...
- Approved generic objects the robot can pick: black marker, blue marker, cube, green marker, medicine, nut, pipe, sponge.
...
IMPORTANT: Do not use hyphens (-) in your response.\n` + contextStr + visualContext
      ...
      tools: [
          ...
          function: {
            ...
            parameters: {
              ...
              properties: { 
                ...
                user_context: {
                  type: "string",
                  description: "Optional visual context (e.g., 'milk sticker' or 'umbrella'). Supply this if the user needs a specific cube or item based on visual branding found in analyze_scene_semantics."
                }
              },
        ...
        },
        {
          ...
          function: {
            ...
            description: "Crops currently detected objects and uses a VLM to describe them. Call this BEFORE locate_object if the user request involves stickers, brands, or implicit visual reasoning (like thirsty -> milk).",
            parameters: {
              type: "object",
              properties: {}
            }
    ...
    if (toolCallsToProcess.length > 0) {
      ...
      for (const toolCall of toolCallsToProcess) {
        ...
        if (toolCall.name === "switch_avatar") {
        ...
        else if (toolCall.name === "analyze_scene_semantics") {
          logToolCall(question, "analyze_scene_semantics", args, "Requesting VLM description of cropped objects...");
          try {
            const response = await visionMcpClient.callTool({ name: "analyze_scene_semantics", arguments: args }, undefined, { timeout: 30000 });
            let textRes = "";
            if (response.content && response.content.length > 0) {
                textRes = response.content[0].text;
            }
            logToolCall(question, "analyze_scene_semantics", args, "Semantics mapped: " + textRes);
            messages.push({
              role: "function",
              name: "analyze_scene_semantics",
              content: textRes
            ...
            logToolCall(question, "analyze_scene_semantics", args, "Error: " + e.message);
            messages.push({
              role: "function",
              name: "analyze_scene_semantics",
          ...
          if (visionMcpClient) {
            ...
            visionMcpClient.callTool({ 
              name: "locate_object", 
              arguments: { target_name: args.target_name, user_context: args.user_context } 
