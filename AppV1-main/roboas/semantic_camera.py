...
@mcp.tool()
def get_object_crops() -> list:
    """
    Runs YOLO inference on the current frame and returns a list of dictionaries
    containing cropped base64 images for each detected object.
    """
    global current_rgb_frame, model
    if current_rgb_frame is None:
        return []
        
    crops = []
    with inference_lock:
        results = model(current_rgb_frame, verbose=False, agnostic_nms=False, iou=0.35, conf=0.35)
    ...
    for result in results:
        ...
        for idx, obb in enumerate(result.obb):
            ...
            # OBB returns 4 corner points
            (x1, y1), (x2, y2), (x3, y3), (x4, y4) = obb.xyxyxyxy[0].cpu().numpy().astype(int)
            
            # Calculate bounding rectangle
            min_x = max(0, min(x1, x2, x3, x4))
            max_x = min(current_rgb_frame.shape[1], max(x1, x2, x3, x4))
            min_y = max(0, min(y1, y2, y3, y4))
            max_y = min(current_rgb_frame.shape[0], max(y1, y2, y3, y4))
            
            # Validate crop dimensions
            if max_y > min_y and max_x > min_x:
                crop = current_rgb_frame[min_y:max_y, min_x:max_x]
